...
module sobel_engine #(
    ...
    // Read port to private frame buffer                    // pfb ports
    output  reg                         rd_req          ,   // eng_rd_req
    output  reg     [ADDR_W-1:0]        rd_addr         ,   // eng_rd_addr
    input   wire    [       7:0]        rd_data         ,   // eng_rd_data
    input   wire                        rd_data_valid   ,   // eng_rd_valid

    // Write port to private frame buffer
    output  reg                         wr_en   ,           // eng_wr_en
    output  reg     [ADDR_W-1:0]        wr_addr ,           // eng_wr_addr
    output  reg     [       7:0]        wr_data ,           // eng_wr_data
    ...
    localparam [1:0] S_BORDER   = 2'd2;
    localparam [1:0] S_DONE     = 2'd3;

    reg [1:0] state;

    // -------------------- BORDER phase ------------------
    reg [1:0] border_phase;
    reg [7:0] border_idx;
    ...
    */
    reg         last_req_sent;      // 最后一个读请求已经发出
    reg         final_resp_seen;    // 最后一个读数据已经收回
    reg [2:0]   drain_cnt;
    ...
    sobel_datapath_pipeline #(
        ...
        .valid_in   (win_valid),
        .addr_in    (win_addr),
        .p00(r0_s0), .p01(r0_s1), .p02(r0_s2),
        .p10(r1_s0), .p11(r1_s1), .p12(r1_s2),
        .p20(r2_s0), .p21(r2_s1), .p22(r2_s2),
        .threshold  (threshold),
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_PROC: begin
                    // 1) 发读请求：rd_addr_cnt 只表示“下一个要发出的地址”
                    if (!last_req_sent) begin
                        rd_req  <= 1'b1;
                        rd_addr <= rd_addr_cnt;
                    ...
                    // 2) 收到读回数据后，才更新窗口 / line buffer / x,y
                    if (rd_data_valid) begin
                        ...
                        // 当前收回的是最后一个输入像素
                        if ((x_cnt == IMG_W - 1) && (y_cnt == IMG_H - 1)) begin
                            final_resp_seen <= 1'b1;
                            drain_cnt       <= 3'd5;
                        end
                    ...
                    if (final_resp_seen) begin
                        if (drain_cnt != 3'd0) begin
                            ...
                            state <= S_BORDER;            // This is double insurance
                        end
                    end
                ...
                S_BORDER: begin
                    ...
                    case (border_phase)
                        2'd0: begin
                            // top row
                            wr_addr <= border_idx;
                            if (border_idx == IMG_W - 1) begin
                                border_idx      <= 8'd0;
                                border_phase    <= 2'd1;
                            end else begin
                                border_idx <= border_idx + 1'b1;
                            end
                        end

                        2'd1: begin
                            // bottom row
                            wr_addr <= (IMG_H - 1) * IMG_W + border_idx;
                            if (border_idx == IMG_W - 1) begin
                                border_idx      <= 8'd0;
                                border_phase    <= 2'd2;
                            end else begin
                        ...
                        2'd2: begin
                            ...
                            wr_addr <= (border_idx + 1'b1) * IMG_W;
                            if (border_idx == IMG_H - 3) begin
                                border_idx      <= 8'd0;
                                border_phase    <= 2'd3;
                            end else begin
                                border_idx <= border_idx + 1'b1;
                        ...
                        2'd3: begin
                            // top row
                            wr_addr <= (border_idx + 1'b1) * IMG_W + (IMG_W - 1);
                            if (border_idx == IMG_H - 3) begin
                                state   <= S_DONE;
                            end else begin
                                border_idx <= border_idx + 1'b1;
